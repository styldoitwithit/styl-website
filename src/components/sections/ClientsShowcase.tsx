...
const SLIDES: Slide[] = [
  {
    ...
    logo: '/assets/ATLAS HOSPITALS.png',
    quote:
  ...
  },
  {
    ...
    logo: '/assets/MUKESH ARTHRO CARE HOSPITAL.png',
    quote:
  ...
  },
  {
    ...
    caseStudy: {
      challenge: 'High staff turnover and inconsistent service delivery impacting patient satisfaction scores.',
      strategy: 'Introduced a structured training programme and internal performance framework across all departments.',
      result: 'Staff retention improved significantly and patient satisfaction scores reached an all-time high.',
      metrics: [
  ...
  },
  {
    ...
    caseStudy: {
      challenge: 'Limited digital presence in a competitive fertility care market with strong word-of-mouth dependency.',
      strategy: 'Developed a comprehensive SEO and social media content strategy emphasising patient education and trust-building.',
      result: 'Online enquiries grew substantially within 90 days, establishing a strong digital brand in fertility care.',
      metrics: [
...
function LogoBox({ slide, size }: { slide: Slide; size: 48 | 64 }) {
  return (
    ...
    >
      {slide.logo ? (
        <Image src={slide.logo} alt={slide.name} fill style={{ objectFit: 'contain' }} sizes={`${size}px`} />
      ) : (
...
export function ClientsShowcase() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const ro = new ResizeObserver(([e]) => { setContainerW(e.contentRect.width); setReady(true); });
    ro.observe(el);
  ...
  const onTransitionEnd = useCallback(() => {
    if (pos < COUNT) { setSmooth(false); setPos(pos + COUNT); }
    else if (pos >= COUNT * 2) { setSmooth(false); setPos(pos - COUNT); }
  }, [pos]);

  const handlePrev  = () => setPos(p => p - 1);
  const handleNext  = () => setPos(p => p + 1);
  const goToDot     = (i: number) => { setSmooth(true); setPos(COUNT + i); };
  const goToCard    = (loopIdx: number) => { setSmooth(true); setPos(COUNT + (loopIdx % COUNT)); };
  ...
  return (
    <section
      style={{ background: '#0a0a0f', paddingTop: 30, paddingBottom: 30, overflow: 'hidden' }}
    ...
    >
      ...
      <div style={{ position: 'relative', paddingBottom: 48 }}>
        ...
        <div
          ref={viewportRef}
          style={{ overflow: 'hidden', width: '100%', cursor: 'grab', opacity: ready ? 1 : 0, transition: 'opacity 200ms' }}
        >
          <div
            ...
            style={{
              ...
              transition: smooth ? 'transform 0.55s cubic-bezier(0.25,0.46,0.45,0.94)' : 'none',
            ...
            {LOOPED.map((slide, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* 4. Stars */}
                  <div style={{ color: '#f6c90e', fontSize: '1.1rem', letterSpacing: 2 }}>
                    ★★★★★
                  </div>
                </div>
              );
          ...
          {SLIDES.map((_, i) => (
            <button
              ...
              style={{
                width: 8, height: 8,
                borderRadius: '50%',
                background: i === activeIdx ? '#c9a84c' : 'rgba(255,255,255,0.25)',
                border: 'none',
                cursor: 'pointer',
                padding: 0,
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              >
                {slide.caseStudy.metrics.map((m, i) => (
                  <div key={i} style={{ flex: 1, minWidth: 100, display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div
                      style={{
                        fontFamily: "'Cormorant Garamond', serif",
                        fontSize: '1.5rem',
                    ...
                    >
                      {m.arrow && (
                        <span style={{ fontSize: '1rem', color: m.arrow === '↑' ? '#4ade80' : '#fbbf24' }}>
                          {m.arrow}
                        </span>
                      )}
                      {m.value}
